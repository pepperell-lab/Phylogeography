...
##ML Tree
```{r loadData}
tre <- read.tree("RAxML_bipartitions.160412_RAxML_bestTree_rapidbootsnp.tree")
...
#midpoint
mpt <- midpoint(tre)
isoInfo2 <- split(mpt$tip.label, gsub("\\w+_", "", mpt$tip.label))
...
ggtree(mpt, layout="circular") + geom_tippoint(aes(color=group)) + theme(legend.position="right") 
...
d=d[d$label<100 & !is.na(d$label),]
p1 + geom_text2(data=d, aes(label=label), hjust=-.2)
