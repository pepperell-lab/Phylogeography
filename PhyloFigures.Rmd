...
## Migrations v2 (UN regions used in BEAST)

### Lineage 1
```{r beast2-lin1, echo=FALSE, warning=FALSE}
...
t <- ggtree(tre.g, ladderize=FALSE) + geom_tippoint(aes(color=group)) + 
  geom_tiplab(aes(color=group))
  #geom_text2(aes(subset = isTip, label = geography, color = group), hjust=-0.1)  
  


###code to make pie chart

df <- fortify(l1t)
df <- df[df$isTip == FALSE,]
df <- df[df$posterior >= 0.5,] #remove nodes with posterior prob limit < 0.5 (weren't annotated)
df <- df[,c('node', 'geography.set', 'geography.set.prob')]


df$geography.set.prob = gsub("\\{|\\}", "", df$geography.set.prob)
df$geography.set = gsub("\\{|\\}", "", df$geography.set)

df.long = cSplit(df, splitCols = c("geography.set", "geography.set.prob"), sep = c(",", ","), direction = "long")
df.wide = as.data.frame(dcast(df.long, node ~ geography.set))

df.wide[is.na(df.wide)] <- 0


pies <- nodepie(df.wide, cols=2:(length(df.wide)), alpha=.6)

tp <- inset(t, pies, height=0.03, width=0.03)

#ggsave("C:/Users/Mary/PepLab/data/Phylogeography/UN_discrete_pdfs/161120_lin4_pies.pdf", plot=tp, width=8, height=16)

### end pie tree code

#write.table(l1.dat, file="C:/Users/Mary/PepLab/data/Phylogeography/Phylo_Figures_Data//161111_lin1_migrations.txt", append=FALSE, quote=FALSE, sep="\t", eol="\n", na="na", row.names=FALSE, col.names=TRUE, qmethod="double")
...
d1mod$node_med <- d1mod$node_med * (1/5e-8)
d1mod$parent_med <- d1mod$parent_med * (1/5e-8)
...
p1 <- ggplot(lin1.m) + geom_line(aes(bb, value, colour=variable)) + theme(legend.position=c(.75,.75), legend.title=element_blank()) + xlab("") + ylab("Number") + scale_colour_manual(values=c("red", "green")) + scale_x_reverse()


###BSP

l1.bsp <- read.table("lin1_UN_BSP.data", skip=1, header=T)

l1.bsp$TimeScaled <- l1.bsp$Time * (1/5e-8)

p3 <- ggplot(l1.bsp) + geom_line(aes(TimeScaled, Mean), colour="black") + geom_line(aes(TimeScaled, Median), colour="purple") + geom_line(aes(TimeScaled, Upper), colour="gray") + geom_line(aes(TimeScaled, Lower), colour="gray") + xlab("Years Before Present") + ylab("Eff Pop Size") + scale_x_reverse() + scale_y_log10()


p2 <- ggplot(lin1) + geom_line(aes(bb, l1un/l1), colour = "green") + theme(legend.position = "none") + xlab("") + ylab("Mig/Possible") + scale_x_reverse()
...
ggdraw() +
  draw_plot(tp, 0, .4, 1, .6) +
  draw_plot(p2, 0, .2, 1, .2) +
  draw_plot(p3, 0, 0, 1, .2) +
  draw_plot_label(c("A", "B", "C"), c(0, 0, 0), c(1, 0.5, 0.25), size = 15)
...
```{r beast2-lin2, echo=FALSE, warning=FALSE}
...
#write.table(l2.dat, file="C:/Users/Mary/PepLab/data/Phylogeography/Phylo_Figures_Data//161111_lin2_migrations.txt", append=FALSE, quote=FALSE, sep="\t", eol="\n", na="na", row.names=FALSE, col.names=TRUE, qmethod="double")
...
d2mod <- d2[!is.na(d2[,'node_med']),]
d2mod <- d2mod[!is.na(d2mod[,'parent_med']),]
d2mod$node_med <- d2mod$node_med * (1/5e-8)
d2mod$parent_med <- d2mod$parent_med * (1/5e-8)
...
p1 <- ggplot(lin2.m) + geom_line(aes(bb, value, colour=variable)) + theme(legend.position=c(.75,.75), legend.title=element_blank()) + xlab("") + ylab("Number") + scale_colour_manual(values=c("red", "green"))


###BSP

l2.bsp <- read.table("lin2_UN_BSP.data", skip=1, header=T)

l2.bsp$TimeScaled <- l2.bsp$Time * (1/5e-8)

p3 <- ggplot(l2.bsp) + geom_line(aes(TimeScaled, Mean), colour="black") + geom_line(aes(TimeScaled, Median), colour="purple") + geom_line(aes(TimeScaled, Upper), colour="gray") + geom_line(aes(TimeScaled, Lower), colour="gray") + xlab("Years Before Present") + ylab("Eff Pop Size")


p2 <- ggplot(lin2) + geom_line(aes(bb, l2un/l2), colour = "green") + theme(legend.position = "none") + xlab("") + ylab("Mig/Possible")
...
```{r beast2-lin3, echo=FALSE, warning=FALSE}

l3t <- read.beast("lin3_UN_MCC.tree")

l3 <- fortify(l3t)

l3.dat <- l3[,c('node', 'parent', 'label', 'geography', 'geography.prob', 'geography.set', 'geography.set.prob', 'posterior', 'height_median')]


l3.dat$geography.set.prob = gsub("\\{|\\}", "", l3.dat$geography.set.prob)
l3.dat$geography.set = gsub("\\{|\\}", "", l3.dat$geography.set)


#write.table(l3.dat, file="C:/Users/Mary/PepLab/data/Phylogeography/Phylo_Figures_Data//161111_lin3_migrations.txt", append=FALSE, quote=FALSE, sep="\t", eol="\n", na="na", row.names=FALSE, col.names=TRUE, qmethod="double")
...
d3mod <- d3[!is.na(d3[,'node_med']),]
d3mod <- d3mod[!is.na(d3mod[,'parent_med']),]
d3mod$node_med <- d3mod$node_med * (1/5e-8)
d3mod$parent_med <- d3mod$parent_med * (1/5e-8)
...
p1 <- ggplot(lin3.m) + geom_line(aes(bb, value, colour=variable)) + theme(legend.position=c(.75,.75), legend.title=element_blank()) + xlab("") + ylab("Number") + scale_colour_manual(values=c("red", "green"))


###BSP

l3.bsp <- read.table("lin3_UN_BSP.data", skip=1, header=T)

l3.bsp$TimeScaled <- l3.bsp$Time * (1/5e-8)

p3 <- ggplot(l3.bsp) + geom_line(aes(TimeScaled, Mean), colour="black") + geom_line(aes(TimeScaled, Median), colour="purple") + geom_line(aes(TimeScaled, Upper), colour="gray") + geom_line(aes(TimeScaled, Lower), colour="gray") + xlab("Years Before Present") + ylab("Eff Pop Size")


p2 <- ggplot(lin3) + geom_line(aes(bb, l3un/l3), colour = "green") + theme(legend.position = "none") + xlab("") + ylab("Mig/Possible")
...
```{r beast2-lin4, echo=FALSE, warning=FALSE}

l4t <- read.beast("lin4_UN_MCC.tree")

l4 <- fortify(l4t)

l4.dat <- l4[,c('node', 'parent', 'label', 'geography', 'geography.prob', 'geography.set', 'geography.set.prob', 'posterior', 'height_median')]


l4.dat$geography.set.prob = gsub("\\{|\\}", "", l4.dat$geography.set.prob)
l4.dat$geography.set = gsub("\\{|\\}", "", l4.dat$geography.set)


#write.table(l4.dat, file="C:/Users/Mary/PepLab/data/Phylogeography/Phylo_Figures_Data//161111_lin4_migrations.txt", append=FALSE, quote=FALSE, sep="\t", eol="\n", na="na", row.names=FALSE, col.names=TRUE, qmethod="double")
...
d4mod <- d4[!is.na(d4[,'node_med']),]
d4mod <- d4mod[!is.na(d4mod[,'parent_med']),]
d4mod$node_med <- d4mod$node_med * (1/5e-8)
d4mod$parent_med <- d4mod$parent_med * (1/5e-8)
...
p1 <- ggplot(lin4.m) + geom_line(aes(bb, value, colour=variable)) + theme(legend.position=c(.75,.75), legend.title=element_blank()) + xlab("") + ylab("Number") + scale_colour_manual(values=c("red", "green"))


###BSP

l4.bsp <- read.table("lin4_UN_BSP.data", skip=1, header=T)

l4.bsp$TimeScaled <- l4.bsp$Time * (1/5e-8)

p3 <- ggplot(l4.bsp) + geom_line(aes(TimeScaled, Mean), colour="black") + geom_line(aes(TimeScaled, Median), colour="purple") + geom_line(aes(TimeScaled, Upper), colour="gray") + geom_line(aes(TimeScaled, Lower), colour="gray") + xlab("Years Before Present") + ylab("Eff Pop Size")


p2 <- ggplot(lin4) + geom_line(aes(bb, l4un/l4), colour = "green") + theme(legend.position = "none") + xlab("") + ylab("Mig/Possible")
...
```{r beast2-lin567, echo=FALSE, warning=FALSE}

l5t <- read.beast("lin5_UN_MCC.tree")
l6t <- read.beast("lin6_UN_MCC.tree")
l7t <- read.beast("lin7_UN_MCC.tree")
...
l5.bsp <- read.table("lin5_UN_BSP.data", skip=1, header=T)
l6.bsp <- read.table("lin6_UN_BSP.data", skip=1, header=T)
l7.bsp <- read.table("lin7_UN_BSP.data", skip=1, header=T)

l5.bsp$TimeScaled <- l5.bsp$Time * (1/5e-8)
l6.bsp$TimeScaled <- l6.bsp$Time * (1/5e-8)
l7.bsp$TimeScaled <- l7.bsp$Time * (1/5e-8)

p5 <- ggplot(l5.bsp) + geom_line(aes(TimeScaled, Mean), colour="black") + geom_line(aes(TimeScaled, Median), colour="purple") + geom_line(aes(TimeScaled, Upper), colour="gray") + geom_line(aes(TimeScaled, Lower), colour="gray") + xlab("Years Before Present") + ylab("Eff Pop Size")

p6 <- ggplot(l6.bsp) + geom_line(aes(TimeScaled, Mean), colour="black") + geom_line(aes(TimeScaled, Median), colour="purple") + geom_line(aes(TimeScaled, Upper), colour="gray") + geom_line(aes(TimeScaled, Lower), colour="gray") + xlab("Years Before Present") + ylab("Eff Pop Size")

p7 <- ggplot(l7.bsp) + geom_line(aes(TimeScaled, Mean), colour="black") + geom_line(aes(TimeScaled, Median), colour="purple") + geom_line(aes(TimeScaled, Upper), colour="gray") + geom_line(aes(TimeScaled, Lower), colour="gray") + xlab("Years Before Present") + ylab("Eff Pop Size")


p5.2 <- ggplot(l5.bsp) + geom_line(aes(TimeScaled, Mean), colour = "black") + geom_ribbon(aes(x=TimeScaled, ymax=Upper, ymin=Lower), fill= "grey", alpha=.2) + xlab("Years Before Present") + ylab("Eff Pop Size") + scale_x_reverse() + scale_y_log10()
...
```{r comb, echo=FALSE, warning=FALSE}

bsp <- rbind(data.frame(lin="lin1", l1.bsp),
              data.frame(lin="lin2", l2.bsp),
              data.frame(lin="lin3", l3.bsp),
              data.frame(lin="lin4", l4.bsp),
              data.frame(lin="lin5", l5.bsp),
              data.frame(lin="lin6", l6.bsp),
              data.frame(lin="lin7", l7.bsp))

colors = c("pink", "blue", "purple", "red", "darkred", "darkgreen", "yellow")

bsp.p <- ggplot(bsp) + geom_line(aes(TimeScaled, Mean, group=lin, colour=lin)) + geom_ribbon(aes(x=TimeScaled, ymax=Upper, ymin=Lower, group=lin, colour=lin, fill=lin), alpha=.2) + scale_color_manual(values = c("pink", "blue", "purple", "red", "darkred", "darkgreen", "yellow")) + scale_fill_manual(values = c("pink", "blue", "purple", "red", "darkred", "darkgreen", "yellow")) + xlab("Years Before Present") + ylab("Eff Pop Size") + scale_x_reverse() + scale_y_log10()
  
bsp.p2 <- ggplot(bsp) + facet_wrap(~lin) + geom_line(aes(TimeScaled, Mean)) + geom_ribbon(aes(x=TimeScaled, ymax=Upper, ymin=Lower), alpha=.2) + xlab("Years Before Present") + ylab("Eff Pop Size") + scale_x_reverse() + scale_y_log10()


mig <- rbind(data.frame(lin="lin1", bb=lin1$bb, tot=lin1$l1, un=lin1$l1un),
              data.frame(lin="lin2", bb=lin2$bb, tot=lin2$l2, un=lin2$l2un),
              data.frame(lin="lin3", bb=lin3$bb, tot=lin3$l3, un=lin3$l3un),
              data.frame(lin="lin4", bb=lin4$bb, tot=lin4$l4, un=lin4$l4un)) 


mig.p <- ggplot(mig) + geom_line(aes(x = bb, y=un/tot, group = lin, colour = lin)) + scale_color_manual(values = c("pink", "blue", "purple", "red")) + theme(legend.position = "none") + xlab("") + ylab("Mig/Possible") + scale_x_reverse()

mig.p2 <- ggplot(mig) + facet_wrap(~lin) + geom_line(aes(x = bb, y=un/tot)) + theme(legend.position = "none") + xlab("") + ylab("Mig/Possible") + scale_x_reverse()
